...
# Aliases accepted at import time, mapped to canonical schema keys.
COLUMN_ALIASES: dict[str, str] = {
    "Ionizable mol%": "ionizable_lipid_pct",
    "Cholesterol mol%": "cholesterol_pct",
    "Helper mol%": "helper_lipid_pct",
    "PEG-Lipid mol%": "peg_lipid_pct",
    "N/P Ratio": "np_ratio",
    "FRR (aq:org)": "flow_rate_ratio",
    "pH Buffer": "ph_buffer",
    "Size (Z-avg)": "size_nm",
    "PDI": "pdi",
    "EE%": "encaps_pct",
...
def _normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Map known aliases (Benjamin-style headers, `observed_<key>`) to schema keys."""
    rename: dict[str, str] = {}
...
def _read_excel_flexible(blob: bytes) -> pd.DataFrame:
    """Try multiple header configurations; return the one with the most known columns."""
    known = set(COLUMN_ALIASES) | set(FEATURE_KEYS) | set(OUTPUT_KEYS)
    ...
    for header in (0, 1, 2):
        try:
            ...
            continue
        score = sum(1 for c in df.columns if c in known)
        if score > best_score:
            best_score = score
            best = df
    if best is None:
        raise ValueError("Could not read Excel file with any header configuration.")
    # Drop a units row if present (cells like '—', 'mol%', 'categorical', 'YYYY', 'mV', 'nm').
    if len(best) > 0:
        first = best.iloc[0].astype(str)
        unit_markers = {"—", "mol%", "categorical", "YYYY", "mV", "nm", "mM", "%", "mL/min", "µg/mL"}
        if first.isin(unit_markers).sum() >= 3:
...
def _handle_excel(uploaded, dataset_id: str) -> None:
    try:
        new_df = _read_excel_flexible(uploaded.read())
    ...
    # Coerce to numeric (drop non-numeric rows that snuck through, e.g. residual unit row)
    for c in FEATURE_KEYS + OUTPUT_KEYS:
